...
import (
	...
	"github.com/iota-uz/devhub/services"
)
...
func (m Model) serviceListView() string {
	...
	// Add MCP connection info box
	if m.MCPServer != nil && m.MCPServer.Port > 0 {
		// Calculate box width based on command length
		mcpCommand := fmt.Sprintf("claude mcp add --transport http devhub http://localhost:%d/mcp", m.MCPServer.Port)
		mcpBoxWidth := len(mcpCommand) + 6 // Add padding
		if mcpBoxWidth < 70 {
			mcpBoxWidth = 70 // Minimum width
		}
		mcpBorderStyle := lipgloss.NewStyle().Foreground(lipgloss.Color("#4ECDC4"))
		
		var mcpBox strings.Builder
		
		// Top border
		mcpBox.WriteString(mcpBorderStyle.Render("╭" + strings.Repeat("─", mcpBoxWidth-2) + "╮"))
		mcpBox.WriteString("\n")
		
		// MCP info
		mcpBox.WriteString(mcpBorderStyle.Render("│"))
		mcpTitle := lipgloss.NewStyle().
			Foreground(lipgloss.Color("#4ECDC4")).
			Bold(true).
			Render("🔌 MCP Server Active")
		mcpBox.WriteString(lipgloss.PlaceHorizontal(mcpBoxWidth-2, lipgloss.Center, mcpTitle))
		mcpBox.WriteString(mcpBorderStyle.Render("│"))
		mcpBox.WriteString("\n")
		...
		cmdLabel := lipgloss.NewStyle().
			Foreground(lipgloss.Color("#868E96")).
			Render("Add to Claude Code:")
		mcpBox.WriteString(lipgloss.PlaceHorizontal(mcpBoxWidth-2, lipgloss.Center, cmdLabel))
		mcpBox.WriteString(mcpBorderStyle.Render("│"))
		mcpBox.WriteString("\n")
		...
		cmd := lipgloss.NewStyle().
			...
			Padding(0, 1).
			Render(mcpCommand)
		mcpBox.WriteString(lipgloss.PlaceHorizontal(mcpBoxWidth-2, lipgloss.Center, cmd))
		mcpBox.WriteString(mcpBorderStyle.Render("│"))
		mcpBox.WriteString("\n")
		...
		// Center the MCP box
		mcpContainer := lipgloss.NewStyle().
			Width(width).
			Align(lipgloss.Center).
			MarginBottom(2)
		
		b.WriteString(mcpContainer.Render(mcpBox.String()))
		b.WriteString("\n")
	}
